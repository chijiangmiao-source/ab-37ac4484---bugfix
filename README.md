...
| 路径 | 内容 |
|---|---|
| `db.sqlite3` | `sessions` 表（含接收位图 BLOB、整文件 SHA-256、过期时间）与 `chunks` 表（每片 SHA-256、大小、落盘路径、`pending/confirmed` 状态；仅 `confirmed` 进入位图） |
| `chunks/<session_id>/00000000.chunk` | 分片正文，序号从 0 开始、8 位零填充命名 |
| `artifacts/<session_id>.bin` | 校验通过后原子发布的成品 |

进程启动时执行 `reconcile`：删除所有 `pending`（传输未完成）行、丢弃文件缺失或尺寸不符的
chunks 行、删除未入账的孤儿分片与残留 `.tmp`、按存活的 `confirmed` 行重建位图 ——
**已确认分片绝不误判为缺失，未确认字节绝不误判为已收**。
...
**并发与在途语义**：只有当分片正文完整接收、通过长度与 SHA-256 校验、`os.replace`
落盘并与位图更新在同一事务提交后，分片才计入 `received_count`。正文仍在传输时
（即使请求头摘要正确）状态接口与 finalize 一律报告该分片缺失，也不会向任何并发
请求返回成功。同序号并发上传中，最先完成校验的请求得到 `201`；携带相同正文的
并发请求等待在途请求结束后得到幂等 `200`；在途请求失败（摘要不符、长度不符、
连接中断）后，持有完整正确正文的请求自动接力为 `201`。任何 `2xx` 对应的分片都
可被状态接口、finalize 与服务重启共同确认；传输中途中止的请求不会在磁盘或
SQLite 中留下任何进度。
...
残留临时文件清理）、结构化错误形态；`test_concurrent_upload.py` 在真实 uvicorn
上用两条独立 TCP 连接与可控流式屏障覆盖并发交错：暂停期间的缺片状态、并发请求
不得提前成功、在途失败后的合法重试接力、中止传输后重启不污染持久化进度。
